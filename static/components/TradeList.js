...
export default {
  ...
  template: `<div class="trade-grid">
    <trade-card v-for="name in Object.keys(groupedTrades)" :key="name" :wallet-name="name" :wallet-cat="cats[name]||'—'" :trades="groupedTrades[name]" @remove="$emit('remove', name)" @detail="(trade,cat)=>$emit('detail',trade,cat)" />
